...
The **fourth set (added now)** covers the seeded flat-rate states whose anchorable figure is the *rate*, not a deduction — **Pennsylvania, Illinois, and Michigan** — via a new `parseFlatRateJurisdiction` parser that anchors the single flat rate from prose ("the income tax rate is 4.95%"), validates it against a plausibility range, and overlays it onto every single-element bracket (which is exactly how a flat tax is stored), plus the personal exemption where IL/MI carry one. Each gets a staggered-cron caller (`refresh-state-{pa,il,mi}.yml`, Apr–May). Only **Ohio** (graduated brackets, no flat rate and no standard deduction) still waits for a bracket-table parser, and the no-income-tax states (**TX, FL**) have nothing to refresh. 563 tests pass (added the flat-rate adapter corpus — PA rate overlay, IL rate + exemption, the no-rate and implausible-rate failures — every parsed fixture validated against the real §7.2 zod schema); `format:check`, `lint`, `typecheck`, `test`, `build`, and the release audit are all green.

- Deferred to later sets (same pattern): Ohio's graduated schedule once a bracket-table parser lands, the Treasury I-bond refresh (the schema exists but no I-bond shard is seeded yet), and the deeper bracket-table parser itself for the multi-bracket jurisdiction sources. Repo setting note: opening PRs from a workflow requires "Allow GitHub Actions to create and approve pull requests" enabled; the workflows use the built-in `GITHUB_TOKEN` (no third-party action, no extra secret).
