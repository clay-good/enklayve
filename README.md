...
| Tax jurisdictions | **45** — 35 income-tax states + DC + 9 no-income-tax | [`data/state-*-income-tax-*.json`](data) |
| Cited dataset shards | **67**, each with a sibling `.sha256` + manifest entry; every `sourceDocument` ≤160 chars (audit-enforced) | [`data/manifest.json`](data/manifest.json) |
| Tests | **865** unit/golden across 62 files, **+18** Playwright e2e | `npm run test` / `npm run test:e2e` |
...
|---|---|
| Take-Home Pay | Your real net pay (federal + FICA + state + local) across every modeled state — 45 jurisdictions today, growing data-only |
...
**865 unit/golden tests across 62 files** (plus 18 Playwright e2e tests) pass today, alongside `format:check`, `lint`, `typecheck`, `build`, the audit, and `wrangler deploy --dry-run`.
...
- **Income-tax states beyond the seeded 45** — added through the staggered annual refresh (35 income-tax states + DC and all nine no-income-tax states already ship). The clean flat-tax states landed fastest (Louisiana's 3% and Iowa's 3.8% reforms, both data-only); same-schedule graduated states followed (Virginia 29th, Missouri 30th, Delaware 34th — a deep seven-tier statutory schedule — Rhode Island 36th, a standard-deduction + exemption stack on a uniform schedule, and West Virginia 39th, its 2026 5% cut over a uniform schedule with no standard deduction); the **federal-taxable-income conformity** states (Idaho, Iowa, **Montana (42nd)** — a two-rate 4.70%/5.65% schedule under HB 337, the largest income-tax cut in Montana history — and **North Dakota (44th)**, a 0%-band-then-1.95%/2.50% schedule under SB 2034, the lowest top rate of any wage-taxing state) compute on federal taxable income, so the federal standard deduction is the starting point; **New Jersey (31st) lifted the per-filing-status limitation**, **Minnesota (32nd), Kansas (33rd), New Mexico (35th), Oklahoma (38th), and Hawaii (41st)** proved it generalizes (Oklahoma a fresh 2026 reform — three brackets over a 0% band, head of household on the joint schedule; Hawaii the deepest schedule yet — twelve brackets whose thresholds derive by a fixed 2×/1.5× ratio), **South Carolina (37th) lifted the sliding-standard-deduction limitation** (H.4216's SCIAD, a deduction reduced *proportionally* with AGI), **Wisconsin (40th) added the second sliding-deduction form** (Wis. Stat. §71.05(23)'s deduction reduced by a *flat percentage* of AGI — 12% single, 19.778% joint), and **Maine (43rd) reused the SC divisor form** (its §5124-C standard-deduction phase-out, plus a new 2026 9.15% millionaire surtax) — so all three deferred *engine* capabilities (per-filing-status, taxpayer credit, and both sliding-deduction variants) ship; and **Vermont (45th)** added the Rhode Island deduction-plus-exemption stack on a per-status graduated schedule (3.35→8.75%, data-only, cited to the official 2025 IN-111 rate schedules confirmed for 2026 by the Tax Foundation table). Still deferred — the **six** remaining income-tax jurisdictions (clearing all six completes every one of the 50 states + DC). Two are now **engine-ready, waiting only on verified 2026 data:** the **federal-income-tax deduction** capability ships (uncapped for **Alabama**'s deductible federal tax, capped-and-AGI-phased for **Oregon**'s federal-tax subtraction; the evaluator subtracts `min(federal tax, cap)` from state taxable income using its own federal figure, so the marginal probe stays honest — synthetic golden cases in [`federalTaxDeduction.test.ts`](tests/engine/federalTaxDeduction.test.ts) pin both shapes, the filing-status fallback, and the schema guards), so each will ship the moment its indexed standard deduction / cap and exemption-credit figures are confirmed. The other four still need their own modeling: states with **mandatory county income taxes** (Maryland's 2.25–3.20% local add-on would materially *under*count without it, and the engine's local add-ons are opt-in, not residence-mandatory), a **high-income benefit recapture plus an income-phased exemption and credit** (Connecticut's 3% recapture, its phasing personal exemption, and its personal tax credit — a bare-bracket figure would be wrong in both directions across the income range), a **bracket-adjustment tax table** (Arkansas's low-income tables and its income-range bracket-adjustment subtraction, plus per-filing personal tax credits), and **Nebraska** (its 2026 head-of-household brackets/standard deduction are not cleanly published and it relies on a flat per-person exemption *credit* the engine does not yet model). Each is held rather than shipped at a wrong number — the project's first principle. See the [state coverage cheat sheet](#state-coverage-cheat-sheet).
