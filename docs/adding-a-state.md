...
**Every tile that computes a state tax charges it**, and that is a change from how this worked until 2026-09-02, when only Take-Home did: the Marginal Rate Explorer, the Paycheck Optimizer, the Benefit Cliff Explorer, the Marginal Reality Rate, Quarterly Taxes, the home anti-budget and the Readout Report all render the same dropdown through [`ui/residenceLocal.ts`](../src/ui/residenceLocal.ts) and pass the id to the engine. If you add a third such state, you need to touch none of them. **Opt-in** locals are the ones still confined to Take-Home, deliberately: those are a question the reader answers about themselves, not a fact about where they live. The chosen county is remembered on the situation profile as `county` — a `localAddOns` id, never a place name, because *Washington* is a county in Maryland, a county in Indiana, and a state — and only a mandatory local is ever written there.
