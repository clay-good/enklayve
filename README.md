...
- **Phase 13 — The home experience.** The three-zone home from SPEC-2 §1: the **Readout dropzone** as the hero, the inline fuzzy search bar (⌘K command palette), and a compact grid of **expandable category cards** — one per pillar plus Your Plan — collapsed by default so the page stays short to scroll. No mega dropdown. A dedicated **All Tools index** (`#/all-tools` in-app) has a static, pre-rendered, crawlable companion at `/tools.html` generated from the registry at build time, so every tool has a stable, linkable, indexable home. axe-core covers the home, the index, and the Readout view with zero violations.
