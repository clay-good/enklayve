...
| Tax jurisdictions | **35** — 25 income-tax states + DC + 9 no-income-tax | [`data/state-*-income-tax-*.json`](data) |
| Cited dataset shards | **57**, each with a sibling `.sha256` + manifest entry; every `sourceDocument` ≤160 chars (audit-enforced) | [`data/manifest.json`](data/manifest.json) |
| Tests | **792** unit/golden across 61 files, **+18** Playwright e2e | `npm run test` / `npm run test:e2e` |
...
|---|---|
| Take-Home Pay | Your real net pay (federal + FICA + state + local) across every modeled state — 35 jurisdictions today, growing data-only |
...
- Seeded with **35 jurisdictions** and growing data-only through the staggered annual refresh (SPEC §14.3). **No-income-tax states are first-class records,** not omissions, so a resident sees their state by name with $0 state tax confirmed (and its citation), not a generic "no state tax modeled."
...
**792 unit/golden tests across 61 files** (plus 18 Playwright e2e tests) pass today, alongside `format:check`, `lint`, `typecheck`, `build`, the audit, and `wrangler deploy --dry-run`.
...
- **Income-tax states beyond the seeded 35** — added through the staggered annual refresh (25 income-tax states + DC and all nine no-income-tax states already ship). The clean flat-tax states landed fastest (Louisiana's 3% and Iowa's 3.8% reforms, both data-only); same-schedule graduated states followed (Virginia 29th, Missouri 30th, and Delaware 34th — a deep seven-tier statutory schedule); **New Jersey (31st) lifted the per-filing-status limitation**, and **Minnesota (32nd), Kansas (33rd), and New Mexico (35th)** proved it generalizes — a state whose single/joint/HoH schedules differ (New Mexico even folds head-of-household into the joint schedule) is now data, not code, with Wisconsin, Oregon, and others next. Still deferred: states with **mandatory county income taxes** (Maryland's 2.25–3.20% local add-on would materially undercount without it, and the engine's local add-ons are opt-in, not residence-mandatory) and states with a large **federal-tax subtraction** (Oregon) or **sliding standard deduction** (Wisconsin) that would need their own modeling before they could ship accurately. See the [state coverage cheat sheet](#state-coverage-cheat-sheet).
