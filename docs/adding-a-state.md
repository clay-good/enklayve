...
A **mandatory residence-based local tax** (the Maryland and Indiana county pattern) **is** supported via the optional `residenceLocalTax` block. Where the opt-in `localAddOns` (NYC, Yonkers, Columbus, Detroit) are a multi-checkbox set a resident *chooses*, a county tax in these two states is mandatory and set by county of residence — exactly one applies. Set `residenceLocalTax: { label, defaultId }` and list the counties as `localAddOns` (a `flatRate`, or — Anne Arundel and Frederick — income-tiered `brackets`); the tiles then render a required single-select dropdown labeled `label`, defaulting to `defaultId`, and the evaluator applies the chosen county's rate to the state's taxable income. No engine change: the local-add-on machinery already applies the selected id, which is why Indiana's 92 counties were a data-only add on top of Maryland's 24. See [`state-md-income-tax-2024.json`](../data/state-md-income-tax-2024.json) and [`state-in-income-tax-2024.json`](../data/state-in-income-tax-2024.json).

**Every tile that computes a state tax charges it**, and that is a change from how this worked until 2026-09-02, when only Take-Home did: the Marginal Rate Explorer, the Paycheck Optimizer, the Benefit Cliff Explorer, the Marginal Reality Rate, Quarterly Taxes and Take-Home all render the same dropdown through [`ui/residenceLocal.ts`](../src/ui/residenceLocal.ts) and pass the id to the engine; the home anti-budget and the Readout Report take the same rule from that module and render (or do not render) their own way. If you add a third such state, you need to touch none of them. **Opt-in** locals are the ones still confined to Take-Home, deliberately: those are a question the reader answers about themselves, not a fact about where they live. The chosen county is remembered on the situation profile as `county` — a `localAddOns` id, never a place name, because *Washington* is a county in Maryland, a county in Indiana, and a state — and only a mandatory local is ever written there.

A **local tax computed on its own base** (the Detroit pattern) **is** supported by putting `personalExemptionByFilingStatus` on the `localAddOns` entry itself. Most localities are levied on the state's taxable income by statute — Indiana's Schedule CT-40 line 1 is literally "the amount from IT-40, line 7" — and those carry no exemption of their own. Detroit does not: the city return starts from AGI and subtracts **$600 per exemption** where Michigan subtracts $5,900, so its add-on carries `{ single: 600, married_jointly: 1200, head_of_household: 600 }` and the evaluator computes that one on `AGI − exemption` (floored at zero) instead. The field is additive and resolved through the usual filing-status fallback; an add-on without it behaves exactly as before. Omitting it where a locality needs it errs **low**, which is the wrong direction for this project — for Detroit it would have understated the city tax by $127.20 on a $60,000 wage. See [`state-mi-income-tax-2024.json`](../data/state-mi-income-tax-2024.json).
