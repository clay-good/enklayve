...
| Tests | **2,023** unit/golden across 89 files, **+25** Playwright e2e | `npm run test` / `npm run test:e2e` |
...
flowchart TD
    ...
    subgraph TILES["src/tiles — 68 calculators in 12 hubs"]
...
- **Accessibility.** axe-core runs inside the test suite across the home, About, All Tools, the Readout, the Report, and **every one of the 68 calculators and 12 hubs** — the roster derived from the registry rather than a hand-kept list, so a tile added tomorrow is covered the moment it lands — with **zero violations**. A **skip-to-content link** (WCAG 2.4.1) is the first focusable element on every page — it focuses the `<main>` directly (no hash navigation), and focus moves into the content region after each route change; visible focus rings throughout, and the command palette is fully keyboard-operable, never traps, and restores focus to the prior element when dismissed. On any **coarse pointer** (a `@media (pointer: coarse)` block), every primary control — buttons, links, the switch-calculator tabs, the All Tools hub headers, every `<select>`, the checkbox rows, and the disclosure toggles — is sized to a **≥44px tap target** (WCAG 2.5.8), so nothing is fiddly under a thumb.
...
**2,023 unit/golden tests across 89 files** (plus 25 Playwright e2e tests) pass today, alongside `format:check`, `lint`, `typecheck`, `build`, the audit, and `wrangler deploy --dry-run`.
...
| `src/tiles` | One module per calculator (68 of them), the hub factory, and the registry |
...
**SPEC-4 — the public-utility pass, in progress.** Specs 1–3 completed the catalog for a household that is *planning*; [SPEC-4](docs/specs/SPEC-4.md) adds **Pillar 4, "Rough Water"** — the safety-net and crisis side, where a raise can cost more in lost benefits than it adds in pay, a hospital bill can be wrong, and the order you pay four bills in decides whether you keep your housing and your job.
