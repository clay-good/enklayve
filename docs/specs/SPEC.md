...
- Deferred to later sets (same pattern): the Treasury I-bond refresh (the schema exists but no I-bond shard is seeded yet, and no tile consumes one), and per-filing-status graduated schedules for any future state whose tiers differ by status. Repo setting note: opening PRs from a workflow requires "Allow GitHub Actions to create and approve pull requests" enabled; the workflows use the built-in `GITHUB_TOKEN` (no third-party action, no extra secret).
