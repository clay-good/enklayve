...
- **Done:** the owed screener's region path (the only surface that lets the user pick Alaska or Hawaii) now pushes a data-honest finding — program "SNAP (food assistance)", estimate "Not estimated here", a note naming the region and pointing to Benefits.gov, and no invented number (citation `null`) — instead of silently dropping SNAP. Pinned by a Hawaii case in [`owedTiles.test.ts`](../../tests/ui/owedTiles.test.ts). **Extended to the standalone SNAP tile on 2026-09-09.** The sentence that stood here — that the tile "takes no region input (contiguous-only by construction)" — was true of its *form* and never of the tile: it has taken household size and income from My Situation since it was built, and My Situation holds the reader's state. So it had the region all along and printed a lower-48 dollar figure to an Alaskan household with the limitation stated only in the explainer, which is the shape the screener was fixed out of in the same week. It now renders the same data-honest refusal, deriving the region with `fplRegionFor` like every other surface that draws a poverty line. It gains no dropdown — two of three entries would lead to "not estimated", which is a question with one usable answer — but the region rides the permalink, because a field the fragment omits is handed to the *reader's* profile and a link built in the lower 48 would otherwise refuse to answer for an Alaskan who opened it.
