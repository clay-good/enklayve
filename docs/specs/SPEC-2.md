...
7. **The home is radically simplified to three calm zones, and the header to a wordmark + a sun/moon (adopted 2026-06-01).** The teaching journey (§0.6) taught well but still read as busy, and the header carried three controls. The product is now spelled out plainly at a roughly 7th-grade reading level and stripped to the essentials:
   - **Header:** left, the wordmark **enklayve** with the lowercase tagline **"personal finance counsel"** beside it; right, a single large **sun/moon toggle** that flips light ↔ dark. The "Search tools" and "My Situation" buttons and the theme dropdown are gone from the header.
   ...
   - **Footer:** a row of **uniform buttons** — My situation, the **High contrast** toggle (the third theme's home, so the header stays a clean binary; SPEC §10), Why enklayve, GitHub, and the author credit — that wraps to a tidy two-up grid on a phone.
...
- Deferred to later waves (same pattern): the OCR engine itself (the *flagging* is built; bundling an on-device OCR engine lands with **offline support in Phase 8**, so the pdf.js/worker chunks are service-worker-cached and the "works fully offline" criterion is fully met); and Word (.docx) parsing via mammoth.
