...
490 tests pass (added the self-employed golden/behavior corpus, the W-4 behavior cases, and the chart-render assertions on the budget tiles; every new tile is in the axe sweep with zero violations); `format:check`, `lint`, `typecheck`, `build`, `wrangler deploy --dry-run`, and the release audit are all green.
