...
| Tax jurisdictions | **32** — 22 income-tax states + DC + 9 no-income-tax | [`data/state-*-income-tax-*.json`](data) |
| Cited dataset shards | **54**, each with a sibling `.sha256` + manifest entry; every `sourceDocument` ≤160 chars (audit-enforced) | [`data/manifest.json`](data/manifest.json) |
| Tests | **777** unit/golden across 61 files, **+18** Playwright e2e | `npm run test` / `npm run test:e2e` |
...
|---|---|
| Take-Home Pay | Your real net pay (federal + FICA + state + local) across every modeled state — 32 jurisdictions today, growing data-only |
...
- Seeded with **32 jurisdictions** and growing data-only through the staggered annual refresh (SPEC §14.3). **No-income-tax states are first-class records,** not omissions, so a resident sees their state by name with $0 state tax confirmed (and its citation), not a generic "no state tax modeled."
- **Per-filing-status schedules are supported** (the schema keys brackets by status and `bracketsFor` resolves each, with the QSS→MFJ→single fallback). New Jersey (different rates *and* thresholds by status) and Minnesota (same rates, different thresholds) both use it, so a state whose tiers differ by status is now data, not code.
...
| Graduated, **schedule differs by filing status** | NJ, MN | NJ: single/MFS run a 7-bracket schedule, MFJ/HoH/QSS an 8-bracket one (an extra 2.45% tier); no standard deduction, a $1,000 exemption ($2,000 joint). MN: same four rates (5.35–9.85%) but single/MFJ/HoH thresholds all differ, over an indexed standard deduction. The engine resolves brackets per status — adding such a state is now data, not code |
...
**777 unit/golden tests across 61 files** (plus 18 Playwright e2e tests) pass today, alongside `format:check`, `lint`, `typecheck`, `build`, the audit, and `wrangler deploy --dry-run`.
...
- **Income-tax states beyond the seeded 32** — added through the staggered annual refresh (22 income-tax states + DC and all nine no-income-tax states already ship). The clean flat-tax states landed fastest (Louisiana's 3% and Iowa's 3.8% reforms, both data-only); same-schedule graduated states followed (Virginia 29th, Missouri 30th); **New Jersey (31st) lifted the per-filing-status limitation**; and **Minnesota (32nd)** proved it generalizes — a state whose single/joint/HoH schedules differ is now data, not code, with Wisconsin, Oregon, and others next. Still deferred: states with **mandatory county income taxes** (Maryland's 2.25–3.20% local add-on would materially undercount without it) and states whose 2026 indexed figures the DOR hasn't yet published. See the [state coverage cheat sheet](#state-coverage-cheat-sheet).
