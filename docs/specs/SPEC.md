...
- Deferred to a later wave: the **ACA premium-tax-credit estimator** (needs the per-county benchmark second-lowest-cost-silver-plan data, a genuinely large dataset) and the **FAFSA Student Aid Index + Pell Grant** (needs the full federal-methodology SAI tables seeded). The screener flags ACA eligibility from the FPL% today and will fold in a dollar estimate when the benchmark data lands.
