...
4. **Dignity in the copy.** SPEC §5.3's tone rules apply doubled. No tool in this pillar may imply the user caused their situation, and none may frame the output as a failure state. "Here is what the rule says and what you can do next" — never "you should have." Of the four parts of this bar this was the one with no enforcement; the narrow, unambiguous half of it is now swept by [`tests/ui/dignity.test.ts`](../../tests/ui/dignity.test.ts) over both the static copy of all 81 tiles and the *rendered* output of every Pillar 4 calculator at a range of inputs, since the sentence that would break this is one assembled at runtime rather than a literal in a tile file. It found nothing, which is why it was worth writing while the copy is right. The factual use of the phrase §3.4 names — "you should have received Form 1095-A" — is deliberately spared, and every pattern is held to a sentence it must match so a silent sweep cannot be a broken one.
...
A new check, `checkHarmTier(tiles)` in `scripts/audit-release.ts`, fails the build when any tile with `pillar === "rough"` omits `harmTier`, when a tier-3 tile has an empty `channels`, or when a tier-2 or tier-3 tile's `how` block is missing the advice line.

**On the scope of the advice line, since §3.3 and this section read differently.** §3.3 states the rule for every Pillar 4 tile; the gate binds tier 2 and 3, and `hub.ts` gives a hub a `how` on the same condition. That is deliberate and follows §3.2 — tier 1 is informational and ships on the normal bar — but the narrower rule is the one in the code, in three places, and the difference sent a later reader looking for a copy gap that was not there. All three tier-1 tools carry the substance anyway: the Cliff Explorer and Marginal Reality both close with "not an eligibility determination. Only the agency that runs a program decides who qualifies", which is §3.3's benefits-eligibility form.

**And that wording used to fail the gate.** §3.3 names four domains — legal, tax, medical-billing, benefits-eligibility — and `ADVICE_MARKERS` matched `legal|financial|tax` and the bare phrase "not advice". The two forms it could not see were the two the spec spells out for the highest-harm tools, so the Cliff Explorer's sentence would have been rejected the day that tile went to tier 2, and the only way through would have been to reword good copy at a regex's request. The markers now cover all four, `tests/build/auditRelease.test.ts` reads the domain list out of this section rather than restating it, and every marker in the list must match something — one of the original two had never matched a single tile. It lives beside `checkProvenance` and `checkCitationLength` so every release invariant is readable in one place, but it **runs from the test suite, not the audit CLI** — the CLI executes under plain `node`, which cannot resolve the extensionless TypeScript module graph the tile registry is built from. `tests/build/auditRelease.test.ts` applies it to the real catalog and CI runs `npm run test`, so the gate is no weaker for living there.
...
- *Two composition bugs the unit tests would not have caught, found by reading a real sweep.* The ACA premium tax credit was being added below 100% FPL, where no credit exists — inventing thousands of dollars at the low end and flattening the real step at the 100%-FPL line. And the refundable Child Tax Credit was shown at its cap because the bundled shard carried the cap but not the earned-income phase-in — an overstatement, disclosed in the on-screen "what this leaves out" list rather than hidden. **Closed on 2026-09-09**: the shard carries §24(d)(1)(B)(i)'s threshold and rate, the sweep passes earned income, and the disclosure narrowed to §24(d)(1)(B)(ii)'s alternative for three or more children, which can only raise the figure — so that curve is a floor for those households rather than a ceiling for everyone.
...
**Split into 20a and 20b, both now shipped.** ✅ **20a — Bill Triage**, as the `bill-triage-2026` shard (the ordering framing and consequence language come from the CFPB's own *Prioritizing bills* tool) plus `engine/triage.ts` and the `when-money-is-tight` hub. It needed none of the deadline machinery, so it could be sourced and shipped on its own. ✅ **20b — Life-Event Sequences** shipped after Phase 23b, which is what it was waiting for: the statutory clocks deserved a dedicated sourcing pass against the published regulations rather than being carried along beside a tool that needed none of them.

**Two notes from building 20b.**

- *The sequences shard carries no figure at all, and a test enforces it.* Every dated step names a `windowId` in `enrollment-windows`, which holds the clock and its citation — so no deadline is written twice and none can drift. The test earned its place immediately: the first draft of the prose said "opens a 60-day window" in five places, which is exactly the duplication the rule exists to prevent.
- *A second new test found two shipped bugs.* Nothing checked that a "Related tools" link named a tool that exists, and `cliff-explorer` and `bill-triage` both pointed at `benefits/owed-screener` when the real id is `screener` — buttons that silently landed on a hub's default calculator instead. The same test caught five bad `tileId` references in the new sequences shard before they shipped.
...
1. ✅ Pillar 4 exists as a named pillar with at least Wave A shipped, every tool clearing the §3 four-part admission bar in addition to all existing invariants. **Three of the four parts are now machine-checked**, not three of four reviewed: `checkHarmTier` gates the tier field, the tier-3 channels and the advice line (§3.2, §3.3), and [`dignity.test.ts`](../../tests/ui/dignity.test.ts) sweeps §3.4. Part 1 — federal-floor anchoring with state variance named — stays a per-tile argument made in review, because "is this anchored to a rule that holds everywhere" is not a string.
2. ✅ `checkHarmTier` runs in the test suite over the real catalog and fails a fixture tile that omits a tier, a tier-3 tile with no channel, or a tier-2/3 tile missing the advice line — all three negative fixtures are in [`auditRelease.test.ts`](../../tests/build/auditRelease.test.ts), alongside the §3.3 domain coverage the advice-line markers had been missing (see §7.2).
3. ✅ The Benefit Cliff Explorer reproduces three hand-computed cliff cases as golden tests (ACA edge, Medicaid MAGI edge, SNAP gross-income-test edge), never monetizes Medicaid, and names every program it does not model — the three cases are [`cliffs.test.ts`](../../tests/engine/cliffs.test.ts)'s "golden case 1/2/3".
4. ✅ `sweepResources` is bounded at 400 points, joins the §2.9 property suite, and returns no non-finite value across all 51 jurisdictions × 5 filing statuses. It had been tested only over three filing statuses in one jurisdiction, in the engine's own suite rather than the property suite — the criterion as written was not met. It now sweeps the full space, and checks the derived figures the tile actually shows a reader: every cliff's depth and width, and every field of the marginal-reality result.
5. ✅ Every deadline on the site is produced by `renderDeadline`, carries a citation, and participates in the staleness banner; a UI test asserts it — over the real shard, not a fixture, so a window added later without a citation fails. **Building this found that the staleness banner did not exist.** The loader had always marked a dataset whose effective year fell outside its window, and nothing consumed the flag, so a lapsed shard kept rendering figures that looked exactly as current as the rest. That is the failure the window exists to prevent, and it would have landed first on the Pillar 4 shards pinned at `staleAfterYears: 0`. There is now a site-wide banner above the content on every view, which names the lapsed datasets rather than gesturing at "some data" and renders nothing at all in the healthy case.
6. ✅ Readout v2 answers in the four-part shape for the EOB, medical-bill, and benefits-notice kinds; every check declares its false-positive case; OCR suppresses rule checks; nothing persists and nothing reaches My Situation unconfirmed. Held by [`answer.test.ts`](../../tests/readout/answer.test.ts) — the per-kind extraction, the non-empty `falsePositive` on every registry entry (which `validateRegistry` also refuses at load), and the `source: "ocr"` suppression.
7. ✅ Every new shard is pinned at `staleAfterYears: 0` with a refresh workflow and a [data-sources.md](../data-sources.md) row. **The workflow for these six is a review, not a rewrite**, and that is the finding. The existing `refresh-*.yml` jobs fetch a source and parse figures out of it, which is right for a bracket table and wrong for a shard carrying what the No Surprises Act protects or what a benefits notice must tell you: auto-rewriting the site's most safety-critical sentences from a scraped page is precisely the failure to avoid. So `watch-pillar4-sources.yml` fingerprints the *visible text* of each source quarterly and opens an issue naming the shards whose sources moved. It never edits a shard. Markup churn does not register; a change to the words does; and an unreachable source is reported separately, because "we could not check" and "nothing moved" are different facts.
8. ✅ The Standing Ledger (Path 1) round-trips exactly, its diff is golden-tested against a synthetic version bump, and a user who never exports sees no behavioral change. The first two are [`ledger.test.ts`](../../tests/profile/ledger.test.ts) ("survives serialize and parse bit-for-bit", "the recompute diff against a synthetic dataset bump"); the third is the end-to-end persistence sweep in [`e2e/app.spec.ts`](../../e2e/app.spec.ts), which reads `localStorage`, `sessionStorage` and IndexedDB after a full session and is the enforceable form of "no behavioral change".
