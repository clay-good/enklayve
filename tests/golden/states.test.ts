...
describe("Arkansas (uniform graduated schedule + a high-income BRACKET-ADJUSTMENT recapture)", () => {
  ...
  it("single $90k is below the recapture (no bracket adjustment yet) → $2,871.41", () => {
    const r = evaluateTaxes(
      { filingStatus: "single", wages: 90000 },
      { federal: ds.federal, state: ds.state("ar"), fica: ds.fica },
    );
    // net taxable 87,530 < 94,700: 609.6 + 3.9%·(87,530 − 26,400), recapture 0.
    expect(cents(r.state!.incomeTax)).toBe("2871.41");
  });

  it("net taxable exactly at the $94,700 floor adds no recapture → single $97,170 = $3,136.70", () => {
    const r = evaluateTaxes(
      { filingStatus: "single", wages: 97170 },
      { federal: ds.federal, state: ds.state("ar"), fica: ds.fica },
    );
    // net taxable 97,170 − 2,470 = 94,700 (= thresholdLow): graduated only.
    expect(cents(r.state!.incomeTax)).toBe("3136.7");
  });

  it("single $99k is mid-band: the recapture ramps to a partial $181.15 → $3,385.71", () => {
    const r = evaluateTaxes(
      { filingStatus: "single", wages: 99000 },
      { federal: ds.federal, state: ds.state("ar"), fica: ds.fica },
    );
    // net taxable 96,530: graduated 3,344.67 + 329·(96,530 − 94,700)/3,200 = 3,344.67 + 188.15.
    expect(cents(r.state!.incomeTax)).toBe("3385.71");
  });

  it("single $130k has the full $287.30 recapture — exact vs Act 2's over-$94,700 schedule → $4,638.71", () => {
    const r = evaluateTaxes(
    ...
    expect(cents(r.state!.incomeTax)).toBe("4638.71");
  });

  it("married jointly $60k uses the same schedule with the $4,940 deduction → $1,670.02", () => {
    const r = evaluateTaxes(
      { filingStatus: "married_jointly", wages: 60000 },
      { federal: ds.federal, state: ds.state("ar"), fica: ds.fica },
    );
    // net taxable 60,000 − 4,940 = 55,060: 609.6 + 3.9%·(55,060 − 26,400).
    expect(cents(r.state!.incomeTax)).toBe("1670.02");
  });
...
describe("Nebraska (LB 754 three-bracket schedule; per-status thresholds, federal-conformity-style deduction)", () => {
  ...
  it("single $60k → $2,026.46 (taxable 60,000 − 8,850 = 51,150, into the 4.55% band)", () => {
    const r = evaluateTaxes(
      { filingStatus: "single", wages: 60000 },
      { federal: ds.federal, state: ds.state("ne"), fica: ds.fica },
    );
    // 2.46%·4,030 + 3.51%·(24,120 − 4,030) + 4.55%·(51,400 − 24,120) = 99.138 + 705.159 + 1,241.24.
    expect(cents(r.state!.incomeTax)).toBe("2026.46");
  });

  it("single $20k stays in the 3.51% band → $348.00 (taxable 11,150)", () => {
    const r = evaluateTaxes(
      { filingStatus: "single", wages: 20000 },
      { federal: ds.federal, state: ds.state("ne"), fica: ds.fica },
    );
    // taxable 20,000 − 8,600 = 11,400 (< 24,120): 2.46%·4,030 + 3.51%·(11,400 − 4,030).
    expect(cents(r.state!.incomeTax)).toBe("348");
  });

  it("single $100k climbs well into the 4.55% top band → $3,846.46", () => {
    const r = evaluateTaxes(
      { filingStatus: "single", wages: 100000 },
      { federal: ds.federal, state: ds.state("ne"), fica: ds.fica },
    );
    // taxable 91,400: 804.297 + 4.55%·(91,400 − 24,120).
    expect(cents(r.state!.incomeTax)).toBe("3846.46");
  });

  it("married jointly $60k stays in the 3.51% band (threshold $49,530) → $1,398.11", () => {
    const r = evaluateTaxes(
      { filingStatus: "married_jointly", wages: 60000 },
      { federal: ds.federal, state: ds.state("ne"), fica: ds.fica },
    );
    // taxable 60,000 − 17,200 = 42,800 (< 48,250): 2.46%·8,040 + 3.51%·(42,800 − 8,040).
    expect(cents(r.state!.incomeTax)).toBe("1398.11");
  });

  it("head of household $60k uses the $39,620 threshold → $1,647.88", () => {
    const r = evaluateTaxes(
      { filingStatus: "head_of_household", wages: 60000 },
      { federal: ds.federal, state: ds.state("ne"), fica: ds.fica },
    );
    // taxable 60,000 − 12,600 = 47,400: 2.46%·7,510 + 3.51%·(38,590 − 7,510) + 4.55%·(47,400 − 38,590).
    expect(cents(r.state!.incomeTax)).toBe("1647.88");
  });

  it("married filing separately maps to single exactly → $60k = $2,026.46", () => {
    const mfs = evaluateTaxes(
      { filingStatus: "married_separately", wages: 60000 },
      { federal: ds.federal, state: ds.state("ne"), fica: ds.fica },
    );
    expect(cents(mfs.state!.incomeTax)).toBe("2026.46");
  });
...
describe("flat-rate states", () => {
  const cases: Array<[string, number, string]> = [
    ...
    ["ga", 60000, "2245.5"], // 4.99%·(60,000 − 15,000 std, HB 463)
    ...
    ["id", 60000, "2071.72"], // 5.3%·(60,000 − 16,100 federal std − 4,811 zero band)
