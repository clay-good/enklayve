...
/**
 * How many bytes of the generated chunk each source file is responsible for,
 * indexed to align with the map's own `sources`.
 *
 * **The breakdown used to rank by how long each source file is**, taken from
 * the map's `sourcesContent`, and in this codebase that is close to ranking by
 * how much prose a file carries. Measured on 2026-09-05: **64% of `src/` never
 * reaches the chunk** — comments, types and whitespace — and the ranking it
 * produced was wrong where it mattered most. `src/data/schemas.ts` came first
 * at 70.3 kB of source and is third at 12.6 kB of chunk, because 82% of it is
 * documentation and type declarations that are erased at build time.
 * `src/engine/finance.ts` fell from second to fifth. `src/ui/shell.ts`, which
 * really is the largest thing in there, was listed third.
 *
 * That is the wrong answer from a tool whose entire job is telling someone what
 * to trim when the budget is full — and on the day this was written the budget
 * had 0.3 kB left. Acting on the old ranking meant deleting documentation from
 * the file with the least to give.
 *
 * So each mapped segment is measured in the *generated* file instead: a segment
 * runs from its own column to the next segment's, and that width is charged to
 * the source it points at. The last segment on a line has no successor and is
 * charged nothing rather than guessed at, which understates every file a little
 * and all of them alike. Bytes are minified and pre-gzip, so these are shares
 * of the chunk rather than a claim about any file's compressed cost — gzip
 * prices repeated text across the whole stream, and no per-file split of it
 * would be honest.
 */
export function mappedBytesBySource(mapJson: string): number[] {
  let map: { mappings?: string; sources?: string[] };
  try {
    map = JSON.parse(mapJson) as { mappings?: string; sources?: string[] };
  } catch {
    return [];
  }
  const mappings = map.mappings;
  const sources = map.sources;
  if (typeof mappings !== "string" || !Array.isArray(sources)) return [];

  const B64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  const decode = (chunk: string): number[] => {
    const out: number[] = [];
    let i = 0;
    while (i < chunk.length) {
      ...
      let cont: number;
      do {
        const digit = B64.indexOf(chunk[i++] ?? "");
        // A character outside the alphabet means this is not a mappings string
        // we understand. Stopping returns what was read rather than throwing
        // inside a report.
  ...
  const bytes = new Array<number>(sources.length).fill(0);
  let sourceIndex = 0;
  for (const line of mappings.split(";")) {
    const segments = line.split(",").filter(Boolean).map(decode);
    for (let k = 0; k < segments.length; k++) {
      const seg = segments[k]!;
      // A segment's first field is its column *delta* from the previous one, so
      // the next segment's delta is this one's width. No absolute column is
      // needed, and tracking one only invites it to drift.
      if (seg.length < 4) continue;
      sourceIndex += seg[1] ?? 0;
      if (sourceIndex < 0 || sourceIndex >= bytes.length) continue;
      const next = segments[k + 1];
      const width = next ? Math.max(0, next[0] ?? 0) : 0;
      bytes[sourceIndex] = (bytes[sourceIndex] ?? 0) + width;
    }
  }
  return bytes;
}
...
function shellBreakdownFromBuild(root: string): string[] {
  ...
  const sources = map.sources ?? [];
  // Generated bytes, not source length — see `mappedBytesBySource` for the
  // ranking that measure produced and why it pointed at the wrong file.
  const lines = shellBreakdown(sources, mappedBytesBySource(raw));
