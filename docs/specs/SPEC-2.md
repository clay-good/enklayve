...
**All Tools index lists every calculator (2026-06-04):** after the hub consolidation (§17 close-out), both the in-app `#/all-tools` view and the static `tools.html` listed only the **10 hubs**, so the *browse* path (as opposed to ⌘K search) could no longer reach a calculator by name — "Roth Conversion Ladder," "EITC," "Refinance Break-Even" were invisible unless you knew their hub. Both surfaces now render **one section per hub with every calculator it hosts listed beneath** (in-app: deep-linking into the hub already switched to that tool via `?tool=`; static: linking each calculator's pre-rendered `/tools/<id>.html` landing page, so all 63 crawlable pages are reachable in one hop from the index rather than only via `sitemap.xml`). The pillar grouping was dropped in favor of the hubs themselves (the unit the app navigates to), removing the duplicate "Investing → Investing" headings the 1:1 pillar/hub mapping produced. A source-order CSS bug was fixed in passing (`.tile-link--stacked` was defined before `.tile-link`, so the base row's `align-items: center` won and centered the entries on desktop). The drift test now asserts the static index names every sub-tool and links its landing page; the in-app view stays axe-clean and scroll-free at every width. 636 tests; `format:check`, `lint`, `typecheck`, `build`, `wrangler deploy --dry-run`, the audit, and the 12 e2e all green.
...
The **fifth wave (2026-06-02) lands the OCR engine itself** — the last reader in the §2.2 prompt, closing the "ingestion works fully offline, scans included" criterion. `extractText.ts` gains an image branch (`isImageFile`: PNG/JPG/WEBP/BMP/GIF/TIFF by MIME or extension, checked *after* the PDF/docx/text branches so a typed PDF is never sent to OCR) that **dynamically imports tesseract.js** — code-split into its own lazy chunk, loaded only when an image is dropped. The privacy model is the crux and is solved the same way the offline service worker is: tesseract needs its wasm core and a language model at runtime, which normally come from a CDN, so enklayve **vendors the ~2 MB English model** (`public/ocr/eng.traineddata.gz`, the compact `4.0.0_fast` LSTM model) and **emits the worker + wasm core same-origin** under `/ocr/` (a small `ocrAssets` Vite plugin reads them from `node_modules/tesseract.js[-core]`). The worker is created with **`workerBlobURL: false`** so it loads from that same-origin URL and adopts **its own response CSP** — `connect-src 'self'` plus `'wasm-unsafe-eval'`, scoped to `/ocr/*` in `worker/index.ts` — rather than a `blob:` worker that would inherit the page's `connect-src 'none'`. **Every page keeps `connect-src 'none'`;** `/ocr/*` is the second carve-out after `/sw.js`, justified identically (same-origin only, no server endpoint, never touches in-memory data). The assets are lazy and the service worker runtime-caches them on first use, so OCR works offline thereafter and the first visit stays light. OCR text flows through the *same* anchored, revision-pinned engine, so every existing extractor reads a scan with no new code, and the `"ocr"` source flags every field lower-confidence for review. Verified end-to-end in a real browser served with the production CSP headers: a generated pay-stub image is recognized and annualized correctly with **zero CSP/network errors**. (This path is intentionally *not* added to the CI e2e suite — real-image OCR is too slow and non-deterministic to gate on — matching the pattern that browser-only behavior is smoke-verified, not pinned. tesseract.js and the model are Apache-2.0.) 626 tests pass (added the `isImageFile` routing corpus); `format:check`, `lint`, `typecheck`, `test`, `build`, `wrangler deploy --dry-run`, the release audit, and the Playwright e2e suite are all green.
...
**Print support (2026-06-04):** the in-app **Print** action calls `window.print()`, but the app had no print stylesheet, so it would have printed the full site chrome. Added a focused `@media print` block (`src/styles.css`): it hides the header, footer, command palette, back links, and every interactive control (the Download/Print/copy/"try an example" buttons, the Readout dropzone); lays the content out black-on-white at full width; keeps result cards, report sections, and breakdown tables from breaking across pages (`break-inside: avoid`); and prints each citation's URL after its link text in the Report appendix (scoped to `.report-list` so per-line breakdown citations in a tool result stay uncluttered) so a printed report stays verifiable. This applies to any view, so a tool's result card also prints cleanly. A print-media e2e (`e2e/app.spec.ts`) asserts the chrome is hidden and the report body remains. 636 unit + 13 e2e green.
...
Close-out housekeeping (2026-06-02): with every §6.1–6.7 tool live, Phase 17 is marked done and the catalog is complete. A documentation-accuracy pass reconciled the counts the waves had drifted past: the live registry is **55 tiles** (the standalone Zero-Based Budget tile was folded into Budget Overview), the engine is **24 jurisdictions = 14 income-tax states + DC + 9 no-income-tax** (the earlier "16 income-tax states" was an arithmetic slip), and the unit/golden suite stands at **623 tests across 53 files**. The Readout dropzone copy that promised OCR "with offline support" was corrected — offline (Phase 8) already shipped, so OCR is now framed plainly as "coming soon." `format:check`, `lint`, `typecheck`, `test`, `build`, `wrangler deploy --dry-run`, the release audit, and the Playwright e2e suite (responsiveness · offline · smoke) are all green. **Remaining deferrals are deliberate and accuracy-bound** (SPEC §0.4): the i18n string extraction, Idaho/Utah (held until their phase-out-credit and mid-year-rate details can be modeled exactly), and international jurisdictions. (The on-device OCR engine, previously held here for explicit sign-off, shipped in Phase 14's fifth wave — see above.)

Budget consolidation (2026-06-02): the budget is now **one tool, on the home page**. The standalone **Budget Overview** tile (`src/tiles/budgetOverview.ts`) was retired and its registry entry removed, dropping the live catalog to **54 tiles**; the home mini-budget (`homeBudgetWidget` in `src/ui/shell.ts`) was rebuilt into the full and only budget. It now takes an **income** at any **pay frequency** (Weekly / Bi-Weekly / Monthly / Annually), a **filing status** and **state**, and **auto-computes taxes through the same `evaluateTaxes` engine** the Take-Home tile uses (annualizing wages for the engine, then dividing the tax back to the chosen frequency). The user splits the rest across the **living-expense buckets** (Housing, Transport, Food, Debt, All other expenses) and two **investing lines** (Retirement, Brokerage); the screen reports **Total expenses**, **Total investments**, an **honest net income** (income − taxes − expenses), and the **investment rate against both gross and net income**, while a donut fills and "left to assign" falls toward zero. The "Open the full budget" hop is gone (there is nowhere else to go), and the anti-budget note — retitled **"Zero Dollar Based Budget or Anti-Budget"** — moved out of the retired tile to close the home budget. The orphaned category-chip and drag-to-reorder CSS was removed with the tile. `format:check`, `lint`, `typecheck`, and the 617-test suite are green.

Budget readability pass (2026-06-02): a delight + usability polish on the home budget. The **state dropdown now lists all 50 states + DC** (`US_STATES` in `src/ui/shell.ts`); the 24 modeled jurisdictions compute state income tax, and a chosen state whose income tax isn't modeled yet shows an **honest amber note** ("we don't model X's state income tax yet, so this shows federal + FICA only") rather than silently under-counting. Money fields gained a **leading "$"** that lights up on focus, income and pay-frequency share one line with the **annual equivalent restated underneath** ("That's about $60,000 a year"), the donut center reads the pay period ("a month"), donut slices round to whole dollars, the two **investment rates are hero cards** (gross + net, side by side), and a **calm green ring** settles around the donut when every dollar has a job. The card widened to 46rem. `format`, `lint`, `typecheck`, the 641-test unit suite, `build`, the 11 e2e, and the release audit are all green.

Anti-budget copy + auto-expand + ZIP spec (2026-06-02): the home budget's **"Zero Dollar Based Budget or Anti-Budget"** note gained two paragraphs that lock in the *act on it* half of the idea: automate every contribution/bill/sweep ("a small, tireless robot whose only job is to make you richer while you sleep"), then pour money in the right order (full employer match, kill high-interest debt, six months into a HYSA, then 401(k) → traditional IRA → HSA → brokerage, then "whatever future you believe in: a rental property, index funds, a mint-condition Charizard, or a tasteful parcel of land in whatever digital world you have decided is going to win"). Funny, but every claim is real personal-finance order-of-operations. Separately, **every `<details>` now opens by default** ("auto-expand all pages"): the result-card breakdown (its summary relabeled the neutral "The math"), the My Plan step math + "My Situation & plan settings", and Peace of Mind's config — so the math and the options are front and center, not hidden behind a tap. On ZIP-code taxes: the product keeps the **state dropdown** (the honest granularity for the data we have); a ZIP-level local-tax design was briefly drafted and then dropped as out of scope.

Home-grid removal + take-home fix + plainer copy (2026-06-02): the home **tool grid was removed** so the page leads with the live search box; tools are reached through search (the `SEARCH_ENTRIES` dropdown) and the **All Tools index** (`#/all-tools`, still SEO-mirrored by `tools.html` + the per-sub-tool pages), now linked from the footer instead of tiled on the home. The budget's net-income metric was **corrected to take-home pay**: "Net income (after tax & expenses)" became **"Take-home pay (after taxes)"** = income − taxes, and the second hero metric is now **"Investment rate, of take-home pay"** = investments ÷ (income − taxes); "Left to assign" is unchanged (income − taxes − expenses − investments). The anti-budget note was **rewritten shorter and at a ~7th-grade level**, retitled **"The anti-budget: give every dollar a job,"** and split into three labeled parts (why it works, make it automatic, fund things in this order). The orphaned `.home-tools*` CSS was removed with the grid.

Home polish + the sideways-scroll fix (2026-06-02): the **"Show me my next step" CTA** and the **home search box** were both enlarged, and the "Not sure where to begin? See your plan" hint was removed from the bottom of the home (with its orphaned `.home-start*` CSS). The real fix this round is horizontal scroll: result-card breakdown **values now wrap** (`.bd-value` was `white-space: nowrap`, so tiles that put a whole sentence in the value column — the W-4 Withholding & Refund Check most of all, now that the math auto-opens — forced an inner sideways scroll). `.bd-value` uses `overflow-wrap: anywhere` (shrinks the cell so the table fits the phone) and `.bd-label` uses `overflow-wrap: break-word` (wraps at spaces, never mid-word). A new e2e (`responsive.spec.ts`) asserts the W-4 breakdown has zero sideways overflow at 360/414/768 px. 641 unit + 12 e2e green.

Landscape-orientation coverage (2026-06-06): the responsive e2e swept eight widths but always at `height: 800` — portrait. A phone *in landscape* is a short viewport, a genuinely different layout regime (the sticky header eats more of the screen, and the ⌘K palette is a fixed overlay that could be clipped off-screen). Added a `responsive.spec.ts` case that, at 740×360 and 932×430, asserts the home + a couple of tools still scroll vertically only, **and** that the open command palette's panel stays within the viewport (its list is capped at 60vh with internal scroll, so the input and results are always reachable). Verified clean — and an ad-hoc check confirmed ultrawide (2560px) has no overflow either (the content column caps at 64rem and centres). 656 unit + 15 e2e green.

Bring the Readout's supported form revisions current (2026-06-05): the anchored extractors validated only tax-year **2023 and 2024** form revisions, so in 2026 a user dropping their **2025 W-2** (the form filed this very season) had it recognized as a W-2 but every field dropped as an "unvalidated revision," forcing full manual entry — a real degradation, since the extractors anchor on box numbers/labels (W-2 boxes 1/2/12/16/17, the 1040 lines, the 1099 series, 1095-A, 1098) that are **unchanged across these years**. Extended `SUPPORTED_REVISIONS` to `["2023","2024","2025","2026"]` (the two current filing years plus the prior two) with a regression test that a 2025 W-2 is recognized, its fields read, and cited to effective year 2025; the existing old-revision guard (a 2009 W-2 is still flagged, not guessed) is untouched, so a genuinely renumbered future form must still be re-validated and added deliberately. 673 unit + 17 e2e green.

Fix the screener's ACA finding for the restored 400% cliff (2026-06-05): when the bundled data rolled to plan year 2026, the ACA tile and the engine were updated for the **returned 400%-FPL subsidy cliff** (the ARPA/IRA enhancement expired after 2025), but the **What Am I Owed? screener** was missed — it still flagged "ACA marketplace subsidies (likely)" for *any* income ≥ 100% FPL, with copy claiming the "enhanced rules through 2025 remove the old 400% cliff." For 2026 that is doubly wrong: above 400% FPL there is no premium tax credit, so a single filer at, say, 575% FPL was promised a subsidy that doesn't exist. Capped the finding at **100–400% FPL** (matching the engine's `aboveSubsidyCap` and the ACA tile) and corrected the note. Added two `owedTiles.test.ts` regression cases (fires within the band; does **not** fire above 400% FPL). Also reconciled a stale `AcaSchema` docstring that still described the top band as open-ended "(no 400%-FPL cliff)." 672 unit + 17 e2e green.

Cover the remaining Readout states in the responsive e2e (2026-06-05): the responsive sweep measured every route plus the Readout confirm + summary, but the Readout's *other* post-drop states render only after a file is dropped and were never measured — the unrecognized-document warning note, the **encrypted-restore unlock row** (a passphrase input + "Unlock & restore" button side by side, a classic phone-overflow source), the wrong-passphrase error sentence inside that row, a successful restore summary, and the **Report rendered from the restored profile** (its populated tables/standing grid). Added a `responsive.spec.ts` case driving all five through the real restore path (a dropped `.json` — no crypto fixture needed, since `isEncrypted` keys only on the envelope's `format` field) at 320/360/414 px. Verified clean: zero sideways overflow in every state. 670 unit + 17 e2e green.

Lock the on-device/US-only/not-advice promise (§0.2) under test (2026-06-05): the promise is rendered once per tool route by the shell's shared `tileExplainer`, **not** by the tiles — so the per-tile axe sweep (which mounts tiles directly) never exercised it, and a refactor of the route renderer could silently drop a §0.2-required clause with nothing failing. Added `tests/ui/promise.test.ts`: a registry-driven sweep that renders every routable tile (all 10 hubs from `TILES`, since every calculator is hosted in a hub at `?tool=`) plus a sample of sub-tool deep links through the real `renderTileView` and asserts the `.explainer-promise` carries all three clauses (computed *on your device*, *U.S.*-scoped, information *not advice*). A new hub is covered automatically. Confirmed it genuinely guards the invariant (neutralizing the promise class fails all 14 cases). 670 unit + 16 e2e green.

Retire My Plan + the My Situation panel (2026-06-02): the home budget already *is* the plan in written form (your situation, captured live in the numbers, plus the anti-budget order-of-operations in plain English), so the separate **My Plan** tile (`src/tiles/yourPlan.ts`) and the **My Situation panel** (`src/ui/situationPanel.ts`) were deleted. The hero "Show me my next step" CTA and its note are gone; the home leads straight with the budget + search. The footer's **"My situation"** button is removed. The anti-budget note gained a closing part, **"Just do the next thing"** ("you do not need a separate dashboard... find the first line you have not finished and do only that one"), folding My Plan's single-next-step value into the copy. Crucially, the **`SituationStore` (the in-memory shared profile) stays** — it is invisible plumbing ~34 tiles use so a value entered once prefills the rest — as does the **plan engine** (`src/engine/plan.ts`), which still powers the downloadable Readout Report; the Readout's "See My Plan" links were repointed/removed. Orphaned CSS removed (`.hero-cta*`, all `.situation-*`, and My Plan's `.plan-step/list/config/intro/open/subhead/next-*child` rules; `.plan-next` is kept since RMD reuses it, and `.plan-debt*` since three tiles do). All tools are unaffected and remain reachable via search and the All Tools index. Tests trimmed accordingly (deleted `yourPlan.test`; trimmed accessibility/smoke/reportView/profileIntegration/hub, keeping the profile-continuity coverage). 625 unit + 12 e2e + release audit green.

Stale-copy follow-up (2026-06-04): retiring the panel left two user-facing strings pointing at it — the Readout summary ("Open My Situation in the header to review or export them") and Peace of Mind's config note ("Open My Situation in the header to export a private copy") — both telling the reader to open a control that no longer exists. Corrected to describe the actual behavior: confirmed Readout values flow into the shared session profile, prefill the matching tools, feed the report, and clear on unload; nothing is uploaded. The portable, passphrase-encryptable export (`src/profile/portable.ts`, §3.2/§5.2) lost its only entry point when the panel was deleted — the module (and its tests) are retained, ready to re-surface, but it was **not wired to any button**, so the README's profile-export wording was reconciled to say so rather than imply a reachable feature. No tool behavior changed.

Export/import re-surfaced (2026-06-04): the orphaned portable export now has a home that matches the spec — **Phase 16's prompt asks to "offer the portable encrypted profile export alongside [the report],"** and §5.2 places it there. The Report view (`src/ui/reportView.ts`, `#/report`) gained a **"Keep a private copy"** section: a passphrase field (optional), a **"Save my situation (.json)"** button (`exportProfile` → plain JSON, or PBKDF2 → AES-GCM when a passphrase is given), and a **"Restore from a file"** input (`importProfile`) that detects an encrypted envelope and reveals an "Unlock & restore" button only then, re-rendering the report from the restored profile on success. It deliberately does **not** resurrect the retired My Situation panel; it puts the already-built, already-tested feature in its spec'd home and keeps the Readout's document-extraction path untouched. The Readout summary copy now points here ("open the report to … save a private copy you can restore later"). All on-device, so `connect-src 'none'` is untouched; the control is axe-clean and scroll-free at every width. 639 unit (added 3 reportView cases — controls present, a plain round-trip restore, and passphrase-prompt-only-when-encrypted) + 13 e2e + audit green.

Restore on the way back in (2026-06-05): export lived only in the Report, but a returning user lands on the home, whose hero is the **Readout dropzone** — so restore needed to be reachable there too. `handleFile` now routes a dropped **`.json`** (by extension or `application/json`) to a restore path instead of document extraction — unambiguous, since no document the extractors read is JSON, so the extraction path is untouched. A plain file imports straight in and shows a **"Restored your situation — N values"** summary reusing the same §2.3 standing block (effective rate, take-home, next step); an encrypted file reveals a passphrase prompt + "Unlock & restore"; a `.json` that isn't a saved situation gets a friendly error. The dropzone copy and `accept` now advertise it ("Already have a saved situation? Drop its .json here to restore it."). Verified the full cross-view round-trip in a real browser (save in the Report → drop the file on the Readout → restored, 0 errors, 0 horizontal overflow at 390px). 642 unit (added 3 readoutView cases — `.json` routes to restore without parsing, encrypted prompts, invalid `.json` errors) + 13 e2e + audit green.

Remove the home search box + simplify the tab title (2026-06-02): the home **search box was deleted** (`homeSearch`/`searchIcon` and the `.home-search*` CSS, plus the now-unused `fuzzyFilter`/`SEARCH_ENTRIES` imports and the orphaned `svgEl`/`iconSvg` SVG helpers in `shell.ts`). Tools are reached via the **All Tools index** (footer link) and the **⌘K command palette** (which still searches `SEARCH_ENTRIES`). The home is now just hero → Readout dropzone → budget → anti-budget note. The **browser-tab title is plain "enklayve"** now: `renderHome` sets `document.title = "enklayve"` (was the "...free, private money tools..." tagline) and `index.html`'s `<title>` matches; per-page titles ("Take-Home Pay · enklayve", etc.) and the OG/social titles are unchanged.

Idaho added + documentation reconciled (2026-06-03): with its mid-year rate cut settled into a clean **5.3% flat tax** (HB 40, 2025), **Idaho** ships data-only on the Colorado pattern — a single bracket over a federal-conformity standard deduction, cited to the Idaho State Tax Commission and golden-tested (single $60k → $2,326.70) — bringing the engine to **25 jurisdictions** (15 income-tax states + DC + 9 no-income-tax). **Utah stays held**: HB 106 (2025) gave it a clean 4.5% flat rate, but its deduction is a 6%-of-federal-deduction taxpayer tax credit that phases out with income, which the bracket-plus-standard-deduction model cannot represent without overstating low-income tax (it waits on an exemption-credit engine feature). The same pass reconciled the documentation counts the home/hub redesigns had drifted past: the live catalog is **10 topic hubs hosting 53 calculators** (My Plan and the standalone Budget Overview were retired as tiles — the home budget *is* the plan, and the `plan.ts` engine still powers the Readout Report), **42 cited dataset shards**, and **626 unit/golden tests + 12 Playwright e2e**. The README's "three calm zones" home sketch, the per-area tool tables, and the state cheat sheet (KY 3.5%, MA surtax over $1,107,750, Idaho flat) were all brought current. `format:check`, `lint`, `typecheck`, `test`, `build`, `wrangler deploy --dry-run`, the release audit, and the e2e suite are all green.

Tool consolidation into hubs (2026-06-02): the catalog now presents **~10 calm topic hubs + My Plan** instead of 53 separate tools, **without rewriting a single calculator**. A new hub factory (`src/tiles/hub.ts`, `defineHub`) returns a `TileDefinition` whose `mount` renders a **segmented control** (a `role="group"` + `aria-pressed` button group, axe-clean) and hosts the active calculator by calling its **existing `mount` unchanged** in a sub-container, via a thin **wrapped `TileContext`**: a reserved `tool` URL key makes a hub view deep-linkable/shareable, `setParams`/`permalink` re-inject it, switching tools resets the URL to a clean slate so one sub-tool's keys never leak to the next, and a sub-tool that navigates to a sibling (e.g. Pell → the FAFSA SAI estimator) is remapped to `#/<hub>?tool=<id>`. The hubs are **Paycheck & Taxes, Self-Employed & 1099, Investing, Retirement, Borrowing & Debt, Budgeting & Cash Flow, Home & Big Purchases, Insurance & Protection, Benefits & Aid, Where You Stand** (`src/tiles/registry.ts`), each hub's first tool its default. The home search and ⌘K palette now query a new **`SEARCH_ENTRIES`** index (every hub + every sub-tool), so searching "EITC"/"refinance" still surfaces the familiar name and **deep-links into its hub already switched to it** (`navigate` widened to `(id, params?)`, `TileContext.navigate` too). The plan engine's step → tile links were retargeted to the hubs (`peace-of-mind`→`where-you-stand`, `retirement-optimizer`→`retirement`, `debt-freedom`→`debt`, all via hub defaults; `sinking-funds`→`budget-cashflow?tool=sinking-fund` via a new optional `StepResult.tool`). SEO crawlability is preserved: `scripts/tool-pages.ts` still emits a static landing page **per hub and per sub-tool** (the sub-tool's "Open" link deep-links into its hub), so the ~53 individual tool pages and sitemap entries remain. The per-tool "How this works"/resources moved to a shared `src/ui/explainer.ts` (`tileHowResources`) the hub renders for its active sub-tool, above the shell's single privacy promise. Every calculator's `mount<Name>`/`<name>Tile` and its unit tests are untouched. `format:check`, `lint`, `typecheck`, the 639-test unit suite, `build`, and the 11-test Playwright e2e suite are all green.
