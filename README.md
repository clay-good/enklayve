...
| Tax jurisdictions | **38** — 28 income-tax states + DC + 9 no-income-tax | [`data/state-*-income-tax-*.json`](data) |
| Cited dataset shards | **60**, each with a sibling `.sha256` + manifest entry; every `sourceDocument` ≤160 chars (audit-enforced) | [`data/manifest.json`](data/manifest.json) |
| Tests | **808** unit/golden across 61 files, **+18** Playwright e2e | `npm run test` / `npm run test:e2e` |
...
|---|---|
| Take-Home Pay | Your real net pay (federal + FICA + state + local) across every modeled state — 38 jurisdictions today, growing data-only |
...
- Seeded with **38 jurisdictions** and growing data-only through the staggered annual refresh (SPEC §14.3). **No-income-tax states are first-class records,** not omissions, so a resident sees their state by name with $0 state tax confirmed (and its citation), not a generic "no state tax modeled."
...
|---|---|---|
| Graduated brackets (one schedule, all statuses) | CA, NY, DC, OH, VA, MO, DE, RI | Ordered marginal tiers; CA adds the 1% mental-health surtax, OH the opt-in Columbus municipal tax, VA stacks a $930 personal exemption on its standard deduction, MO runs eight compressed tiers (0%→4.7%) over a federal-conformity deduction, DE runs seven tiers (0%→6.6%) over a small statutory $3,250/$6,500 deduction with the *same* brackets for joint filers — a built-in marriage penalty. RI (3.75/4.75/5.99%) keeps uniform brackets but stacks *both* a standard deduction and a $5,250-per-taxpayer exemption (the VA stack on a uniform schedule) |
| Graduated, **schedule differs by filing status** | NJ, MN, KS, NM | NJ: single/MFS run a 7-bracket schedule, MFJ/HoH/QSS an 8-bracket one (an extra 2.45% tier); no standard deduction, a $1,000 exemption ($2,000 joint). MN: same four rates (5.35–9.85%) but single/MFJ/HoH thresholds all differ, over an indexed standard deduction. KS: two rates (5.20%/5.58%) crossing at $23,000 single / $46,000 joint (SB 1), over a standard deduction *and* a $9,160/$18,320 exemption. NM: six rates (1.5–5.9%, HB 252) where head-of-household and surviving-spouse filers share the married-jointly schedule (NM has no separate HoH schedule), over the federal-conformity deduction. OK: three rates (2.5/3.5/4.5%, HB 2764 2026) over a 0% band, single thresholds doubled for joint/HoH/surviving-spouse, plus a $6,350/$12,700/$9,350 deduction and $1,000/exemption. The engine resolves brackets per status — adding such a state is now data, not code |
...
| Two brackets + **sliding deduction** | SC (1.99% / 5.21% over $30k) | Starts from federal AGI; the SCIAD deduction ($15k/$30k/$22.5k) phases down with AGI (single $40k→$95k, etc.), the reduction rounded down to the next-lowest $10 (H.4216). Modeled by the engine's `standardDeductionPhaseOut` field |
...
**808 unit/golden tests across 61 files** (plus 18 Playwright e2e tests) pass today, alongside `format:check`, `lint`, `typecheck`, `build`, the audit, and `wrangler deploy --dry-run`.
...
- **Income-tax states beyond the seeded 38** — added through the staggered annual refresh (28 income-tax states + DC and all nine no-income-tax states already ship). The clean flat-tax states landed fastest (Louisiana's 3% and Iowa's 3.8% reforms, both data-only); same-schedule graduated states followed (Virginia 29th, Missouri 30th, Delaware 34th — a deep seven-tier statutory schedule — and Rhode Island 36th, a standard-deduction + exemption stack on a uniform schedule); **New Jersey (31st) lifted the per-filing-status limitation**, **Minnesota (32nd), Kansas (33rd), New Mexico (35th), and Oklahoma (38th)** proved it generalizes (Oklahoma a fresh 2026 reform — three brackets over a 0% band, head of household on the joint schedule), and **South Carolina (37th) lifted the sliding-standard-deduction limitation** (H.4216's SCIAD that phases down with AGI) — so all three deferred *engine* capabilities (per-filing-status, taxpayer credit, sliding deduction) now ship. Still deferred: states with **mandatory county income taxes** (Maryland's 2.25–3.20% local add-on would materially undercount without it, and the engine's local add-ons are opt-in, not residence-mandatory), a large **federal-tax subtraction** (Oregon), and Wisconsin's *different* sliding-deduction form (a flat percentage of income over a threshold, not SC's proportional reduction) — each needs its own modeling before it can ship accurately. See the [state coverage cheat sheet](#state-coverage-cheat-sheet).
