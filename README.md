...
- **Next: Phase 19 — the Benefit Cliff Explorer.** The marquee item, and it needs no new data: FPL, EITC/CTC, ACA, SNAP, Medicaid, and the tax engine are all already bundled and cited, so it's a bounded sweep over shards we ship today. Every incumbent shows the tax marginal rate and stops; nobody shows a working parent that $34,000 → $38,000 can be net-negative, or exactly where the edge sits.
