...
| Tax jurisdictions | **48** — 38 income-tax states + DC + 9 no-income-tax | [`data/state-*-income-tax-*.json`](data) |
...
|---|---|
| Take-Home Pay | Your real net pay (federal + FICA + state + local) across every modeled state — 48 jurisdictions today, growing data-only |
...
- Seeded with **48 jurisdictions** and growing data-only through the staggered annual refresh (SPEC §14.3). **No-income-tax states are first-class records,** not omissions, so a resident sees their state by name with $0 state tax confirmed (and its citation), not a generic "no state tax modeled."
